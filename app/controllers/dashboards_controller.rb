class DashboardsController < ApplicationController
  def index
    ...
    @this_month = @list_this_month.where('extract(month from date) = ?', Date.today.month).sum(:amount)
    ...
    sum = 0
    sum2 = 0
    @per_day = @list_this_month.where('extract(month from date) = ?', Date.today.month).order('extract(day from date)').group(:type_transaction).group('extract(day from date)').sum(:amount)
